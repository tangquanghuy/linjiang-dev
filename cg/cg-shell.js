/* CG 鉴赏页的外壳。
   ...
   escapeHtml 转义、用 toastr 提示、从 currentPhoneData.对象信息 里取好感度、用
...
function getRelationshipDataSource(source = currentPhoneData) {
  const contacts = source && source.对象信息;
  if (contacts && typeof contacts === 'object' && Object.keys(contacts).length > 0) return contacts;
  return null;
}

/* CG 分片现在复用小手机的联系人取值函数。独立页没有加载 phone/src/03-state.js，
   所以在外壳里补齐它实际需要的最小依赖；同时兼容旧 HUD 发来的 {好感度:n} 扁平形状。 */
function getContactBond(contact) {
  if (contact?.羁绊 && typeof contact.羁绊 === 'object') return contact.羁绊;
...
const CGShell = (() => {
  ...
  const onHostMessage = (event) => {
    ...
    if (data.type === `${CHANNEL}:unlock`) {
      ...
      if (unlockCG(unlock.character, unlock.scene, unlock.count)
          && navigationStack.length === 0 && !viewerOpen()) paintRoot();
      return;
    }
    if (data.type === `${CHANNEL}:sync`) {
      const unlocked = data.unlocked && typeof data.unlocked === 'object' ? data.unlocked : {};
      let changed = false;
      Object.entries(unlocked).forEach(([character, scenes]) => {
        if (!scenes || typeof scenes !== 'object') return;
        Object.entries(scenes).forEach(([scene, count]) => {
          if (unlockCG(character, scene, count)) changed = true;
        });
      });
      if (changed && navigationStack.length === 0 && !viewerOpen()) paintRoot();
      return;
    }
    if (data.type !== `${CHANNEL}:data`) return;
    const contacts = data.contacts && typeof data.contacts === 'object'
      ? data.contacts
      : (data.bonds && typeof data.bonds === 'object' ? data.bonds : {});
    currentPhoneData = { 对象信息: contacts };
