...
import {
  ASSETS_ROOT, ARCADE_ASSETS_ROOT, CITY_PLATE_ROOT, listPublicAssets,
  rewriteStaticRefs, rewriteCityPlateRefs, hasWalledCdn,
} from './asset-cdn.mjs';
...
function distFiles(exts = /\.(html|css|js|json)$/) {
    const out = [];
    (function walk(d) {
...
console.log('=== 改写规则（按形态）===');
{
    ...
    eq('city plate path',
        rewriteCityPlateRefs('plate/wuxi_night.webp', new Set(['wuxi_night.webp'])),
        CITY_PLATE_ROOT + 'wuxi_night.webp');
   ...
   在国内就是「大厅一直停在正在载入」。 */
{
  ...
  const shopProducts = readFileSync('dist/shop/products.json', 'utf8');
  if (shopProducts.includes(ASSETS_ROOT + 'items/consumable-medicine.png'))
    ok('shop products.json dynamic icons -> CDN');
  else fail('shop products.json icons still use Pages-relative paths');

  const arcadeRuntime = readFileSync('dist/arcade/token-system.js', 'utf8');
  if (arcadeRuntime.includes(ARCADE_ASSETS_ROOT + 'token/arcade-chip.svg'))
    ok('arcade runtime JS icons -> CDN');
  else fail('arcade token-system.js runtime icons were not rewritten');

  const bgmPages = ['scratch', 'fishing', 'slots']
    .map((name) => readFileSync(`dist/arcade/${name}.html`, 'utf8'));
  if (bgmPages.every((text) => text.includes(ARCADE_ASSETS_ROOT + 'music/') && /assets\/music\/[0-9a-f-]+\.mp3/.test(text))
    )
    ok('original arcade BGM -> repository CDN paths');
  else fail('original arcade BGM was not rewritten to the repository CDN');

  const cityMapJs = readFileSync('dist/city/plate_map.js', 'utf8');
  if (cityMapJs.includes(CITY_PLATE_ROOT + 'overview_night.webp')
    && cityMapJs.includes(CITY_PLATE_ROOT + 'dongtang_night.webp'))
    ok('city plate images -> CDN');
  else fail('city plate images still use Pages-relative paths');
