/* 从 public/shell/aux-shell.js 生成粘贴用的 外部部署/V20260906/辅助计算脚本.js（引导版）。
   ==================================================================
   跟状态栏那次拆分的区别
   ------------------------------------------------------------------
   ...
   **必须同步可用**的消费方：
     ...
     外部部署/V20260906/正文美化.html 的 mountLiveRoom() 在初始渲染路径里就用 lrMenu() 把礼物列表写进
...
const OUTPUT = join(ROOT, '外部部署', 'V20260906', '辅助计算脚本.js');

/* 线上地址。用 jsDelivr 而不是 Pages，理由（延迟差 20 倍的实测）写在
   scripts/build-status-shell.mjs 的 SHELL_URL 那一段，两处是同一个判断。 */
const AUX_URL = 'https://testingcf.jsdelivr.net/gh/tangquanghuy/linjiang-dev@main/public/shell/aux-shell.js';
...
if (same) {
  console.log(`  ok    引导版  外部部署/V20260906/辅助计算脚本.js  ${Buffer.byteLength(body)} 字节`);
} else if (checkOnly) {
  console.error('\n辅助计算脚本引导版已过期，跑 npm run aux:build 重新生成');
  process.exit(1);
} else {
  writeFileSync(OUTPUT, body, 'utf8');
  console.log(`  写入  引导版  外部部署/V20260906/辅助计算脚本.js  ${Buffer.byteLength(body)} 字节`);
}
