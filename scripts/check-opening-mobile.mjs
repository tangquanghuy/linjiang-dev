...
console.log('\n=== opening overlay readiness with stalled decoration ===');
{
  ...
  const closeButton = await page.locator('#linjiang-opening-overlay .lj-close').evaluate((button) => {
    ...
    return {
      ...
      hittable: center === button,
    };
  });
  check(closeButton.top >= 50, 'mobile close button clears the status-bar zone', JSON.stringify(closeButton));
  check(closeButton.width >= 48 && closeButton.height >= 48,
    'mobile close button provides a 48px touch target', JSON.stringify(closeButton));
  check(closeButton.right >= 10 && closeButton.hittable && closeButton.ariaLabel === '关闭开局页',
    'mobile close button stays inset, hittable, and labelled', JSON.stringify(closeButton));
  await page.locator('#linjiang-opening-overlay .lj-close').click();
  check(await page.locator('#linjiang-opening-overlay').count() === 0,
    'mobile close button dismisses the full-screen overlay');
