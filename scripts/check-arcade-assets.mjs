...
for (const rel of ['games/fishing-background.webp', 'games/slots-cherry.webp',
  'music/original-suno.json']) {
  ok(existsSync('arcade/assets/' + rel), `arcade/assets/${rel} 在`);
}
const originalMusic = JSON.parse(readFileSync('arcade/assets/music/original-suno.json', 'utf8'));
for (const track of originalMusic.tracks || []) {
  const path = `arcade/assets/music/${track.file}`;
  const bytes = readFileSync(path);
  const hash = createHash('sha256').update(bytes).digest('hex');
  ok(bytes.length === track.bytes && hash === track.sha256,
    `original Suno ${track.id} mirrored byte-for-byte`, `${track.title} / ${bytes.length} bytes`);
}
for (const [file, game] of [['arcade/scratch.html','scratch'], ['arcade/fishing.html','fishing'], ['arcade/slots.html','slots']]) {
  const text = readFileSync(file, 'utf8');
  const ids = originalMusic.tracks.filter((track) => track.game === game).map((track) => track.id);
  ok(ids.length === 2 && ids.every((id) => text.includes(`assets/music/${id}.mp3`)),
    `${file} keeps both original track IDs in original order`);
}
const lobby = readFileSync('arcade/index.html', 'utf8');
ok(!/id:'auction'|file:'auction\.html'/.test(lobby), 'unfinished auction is absent from lobby routes');
...
async function openGame(path) {
  ...
  return { page, errors: errors.filter((e) => !/favicon/i.test(e)), failed };
}

async function checkBgm(page, label) {
  ...
  ok(!!audio && /assets\/music\/[0-9a-f-]+\.mp3(?:[?#]|$)/.test(audio.src)
    && audio.readyState >= 2 && audio.error === 0,
    `${label} local BGM decoded`, JSON.stringify(audio));
  ok(!!audio && !audio.paused && audio.currentTime > 0,
    `${label} BGM plays after a user gesture`, JSON.stringify(audio));
}

console.log('\n=== arcade/scratch.html ===');
{
  const { page, errors, failed } = await openGame('/arcade/scratch.html');
  await checkBgm(page, 'scratch');
  ok(failed.length === 0, 'scratch has no 4xx/5xx', failed.slice(0, 5).join(' | '));
  ok(errors.length === 0, 'scratch has no script errors', errors.slice(0, 3).join(' | '));
  await page.close();
}
