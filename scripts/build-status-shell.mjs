...
const stampShellVersion = () => {
  ...
  /* 只用内容哈希，**不带日期**。
     第一版是 `<日期>-<哈希>`，结果代码一个字没改、第二天 shell:check 就红了
     （9ad45c7f → 2026-08-31-9ad45c7f）—— 日期让它跨天不幂等，而"重复构建同一份源码得到
     同一个号"正是 --check 能成立的前提。区分版本靠哈希已经够了，日期只是让它变得不确定。 */
  const stamp = `shell-${hash}`;
...
const FLOW_HEADER = `<!-- 状态栏（实验版：收回态改用酒馆原生嵌入）。粘进角色卡「状态栏」。
     ...
     可能残留的 0 高/透明样式，再在当前调用栈立即挂载自己的 HUD；不参加跨楼层 owner 选举，
     也不做交接和旧实例回收。因此本文件与普通版在手机上的启动行为完全一致。

     这个实验开关只保留给桌面/强制抬升路径：收回态使用楼层原生嵌入，展开与全屏仍走原路径。
     全部 RPC action、次级页面、小手机和移动端覆盖层继续共用同一份壳层逻辑。 -->`;
