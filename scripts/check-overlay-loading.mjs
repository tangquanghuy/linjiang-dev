...
try {
  ...
  for (const overlay of OVERLAYS) {
    ...
    const clip = await page.evaluate(() => {
      ...
      if (!r) throw new Error('overlay portal missing');
      return {
    ...
    const closed = await page.evaluate(() => {
      const host = document.getElementById('linjiang-native-overlay-portal');
      const buttons = [...(host?.shadowRoot?.querySelectorAll('.overlay-loading button') || [])];
    ...
    const gone = await page.evaluate(() => !document.getElementById('linjiang-native-overlay-portal'));
