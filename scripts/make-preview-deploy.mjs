...
let mirrorSha = arg('sha', '').trim();
if (!/^[0-9a-f]{40}$/i.test(mirrorSha)) {
  try {
    mirrorSha = git(['ls-remote', `https://github.com/${mirror}.git`, 'refs/heads/main'])
      .trim().split(/\s+/)[0] || '';
  } catch (error) {
    console.error(`unable to read ${mirror} main SHA; retry with --sha=<mirror-sha>`);
    process.exit(1);
  }
}
if (!/^[0-9a-f]{40}$/i.test(mirrorSha)) {
  console.error(`invalid mirror SHA: ${mirrorSha || '(empty)'}`);
  process.exit(1);
}
...
console.log(`      https://testingcf.jsdelivr.net/gh/${mirror}@${mirrorSha}/  media (pinned SHA)`);
