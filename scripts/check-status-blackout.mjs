...
for (const target of TARGETS) {
  ...
  try {
    ...
    /* ---------------------------------------------------------------- 开机静态检查
       桌面抬升架构仍只有最新 owner 占高度；移动端原生流现在刻意绕过 owner/回收，三个楼层
       都要立即画好并保持可见。后者正是“不要再管多个实例，先保证每楼立即出来”的契约。 */
    ...
    const bootOffenders = nativeFlow
      ? staleAtBoot.filter((f) => f.anchorH <= BUDGET.staleAnchorPx || f.stage !== 'painted')
      : staleAtBoot.filter((f) => f.anchorH > BUDGET.staleAnchorPx);
    check(bootOffenders.length === 0,
      nativeFlow ? '开机后每个移动端楼层都已立即画好' : '开机后非 owner 楼层已交出高度',
      JSON.stringify(row.staleAtBoot));
    ...
    for (const [kind, api] of [['rerender', 'rerenderStatusFloor'], ['recreate', 'recreateStatusFloor']]) {
      ...
      check(nativeFlow
        ? (floor.anchorH > BUDGET.staleAnchorPx && floor.stage === 'painted' && !floor.collapsed)
        : floor.anchorH <= BUDGET.staleAnchorPx,
        nativeFlow
          ? `stale(${kind})：复用/重建后的移动端楼层立即恢复并画好`
      ...
      if ((nativeFlow && floor.stage !== 'painted')
        || (!nativeFlow && floor.anchorH > BUDGET.staleAnchorPx)) {
    ...
    for (const order of ['event-first', 'floors-first']) {
      ...
      row.chatSwitch[order] = {
        ...
        candidates: state.manager?.candidates?.length ?? 0,
      ...
      check(!back.timedOut, `chat-switch(${order})：切完对话状态栏还在`,
        ...
        + `候选=${state.manager?.candidates?.length ?? 0} area=${state.sample.area} `
        + `stages=${JSON.stringify(state.sample.stages)}`);
      check(nativeFlow
        ? (state.manager?.ownerRank == null && (state.manager?.candidates?.length ?? 0) === 0)
        : state.manager?.ownerRank != null,
        nativeFlow
          ? `chat-switch(${order})：移动端不依赖 owner 仍保持显示`
          : `chat-switch(${order})：选出了 owner`,
    ...
    row.pileup = {
      ...
      candidates: pileup.manager?.candidates?.length ?? 0,
    };
    check(nativeFlow ? pileup.alive === pileup.floors : pileup.alive <= BUDGET.pileupInstances,
      nativeFlow
        ? 'pileup：移动端每个楼层都保留一份已画好的 HUD'
