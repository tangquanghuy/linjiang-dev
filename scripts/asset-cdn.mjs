...
export const CITY_PLATE_ROOT = `${CDN_HOST}/${CDN_REF}/city/plate/`;
...
const CITY_PLATE_RE = /(^|[\s"'`(,=])((?:\.\.?\/|\/)?)plate\/([A-Za-z0-9._/-]+)/g;
export function rewriteCityPlateRefs(
    text,
    known = new Set(listPublicAssets('city/plate')),
    root = CITY_PLATE_ROOT,
) {
    return text.replace(CITY_PLATE_RE, (match, delim, _prefix, rel) => (
        known.has(rel) ? delim + root + rel : match
    ));
}
...
export function buildRewriteContexts() {
    return [
        {
            id: 'city',
            match: (file) => /(?:^|[\/])city[\/]/.test(file),
            known: new Set(listPublicAssets('public/assets')),
            root: ASSETS_ROOT,
            rewriteMore: (text) => rewriteCityPlateRefs(text),
        },
        {
            id: 'arcade',
    ...
    (function walk(dir) {
        for (const name of readdirSync(dir)) {
            ...
            if (statSync(full).isDirectory()) walk(full);
                ...
                const textAsset = /\.(html|css|json)$/i.test(name);
                /* Raw scripts copied into standalone pages may build image/audio URLs
                   at runtime. Rewrite those too, but never touch Vite's dist/assets
                   bundles: their relative imports are application code, not media. */
                const rawPageScript = /\.js$/i.test(name)
                    && /\/(?:arcade|cg|city)\//.test(normalized);
                if (textAsset || rawPageScript || /\/opening\.js$/i.test(normalized)) out.push(full);
            }
        }
    })(distRoot);
    return out;
