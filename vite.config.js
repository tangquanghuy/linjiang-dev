...
function rewriteAssetRefs(destRoot) {
  ...
  for (const file of listRewritable(destRoot)) {
    const matchFile = file.replace(/\\/g, '/');
    const ctx = contexts.find((c) => c.match(matchFile));
    if (!ctx) continue;
    const before = readFileSync(file, 'utf8');
    let after = rewriteStaticRefs(before, ctx.known, ctx.root);
    if (typeof ctx.rewriteMore === 'function') after = ctx.rewriteMore(after);
    if (after !== before) {
