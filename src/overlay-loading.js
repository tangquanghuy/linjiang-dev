...
export function mountFrameLoading(layer, iframe, { label = '页面', onClose } = {}) {
  ...
  const styleRoot = layer.getRootNode?.() || doc;
  ...
  if (!styleRoot.querySelector?.('[data-overlay-loading-kf]')) {
    const kf = doc.createElement('style');
    kf.dataset.overlayLoadingKf = '1';
    kf.textContent = '@keyframes overlay-spin{to{transform:rotate(360deg)}}';
    if (styleRoot instanceof doc.defaultView.ShadowRoot) styleRoot.appendChild(kf);
    else (doc.head || styleRoot).appendChild(kf);
  }
...
export function warmOverlayPages(urls) {
  ...
  const run = () => {
    for (const url of urls) {
      if (!url) continue;
      /* 必须是**普通的 CORS fetch**，不能加 mode:'no-cors'。
         ==================================================================
         第一版写的是 no-cors，实测无效：真机上仍然是"冷启动第一次打开必黑、重启 App 之后就好"
         —— 也就是页面确实进了磁盘缓存，但预热那一趟没帮上忙。

         原因是 no-cors 拿到的是**不透明响应**（opaque），它在 HTTP 缓存里是单独一份，
         后续 iframe 导航（destination=iframe / mode=navigate）不会复用它。等于白取一遍。

         而 GitHub Pages 是给了 Access-Control-Allow-Origin: * 的（这一点 外部部署/V20260826/
         素材缓存脚本.js 的头部注释里实测记过，它正是靠这个才能把 Pages 上的图收进 IndexedDB）。
         既然给了 ACAO，就该用普通 fetch —— 那样得到的是正常响应，缓存条目可以被导航复用。

         no-store 同样不能加：它强制走网络且不进缓存，那才是真的白预热。默认缓存策略正好。
         失败静默：预热不成功只是回到"点开时再等"，不该有任何用户可见的后果。 */
