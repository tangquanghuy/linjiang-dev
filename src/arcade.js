...
export function isArcadeOpen() {
  return !!activeArcadeLayer?.isConnected || !!document.querySelector('.arcade-layer');
}
...
export function mountArcadeOverlay(host, { onClose } = {}) {
  ...
  root.querySelectorAll('.arcade-layer').forEach((el) => el.remove());
  root.insertAdjacentHTML('beforeend', arcadeOverlay());
  const layer = root.querySelector(':scope > .arcade-layer');
  activeArcadeLayer = layer;
  ...
  if (!mount.portal) document.documentElement.classList.add('has-arcade');
  ...
  let lastPushed = NaN;
  let acceptingArcadeBalance = false;
  let arcadeOwnsBalance = false;
  const cash = () => Math.max(0, Math.round(Number(player.money) || 0));
  const push = ({ force = false } = {}) => {
    /* Once a game has changed the purse, that session is authoritative until the
       overlay closes. Tavern/MVU can briefly publish the previous snapshot while
       its debounced patch is still in flight; pushing that snapshot back caused
       auto-fire balances to bounce and could erase a whale payout. */
    if (acceptingArcadeBalance || (arcadeOwnsBalance && !force)) return;
    const n = arcadeOwnsBalance && Number.isFinite(lastPushed) ? lastPushed : cash();
    lastPushed = n;
    try {
  ...
  const onFrameMsg = (event) => {
    ...
    if (data.type === 'airp-arcade:hello') { push({ force: true }); return; }
    if (data.type === 'airp-arcade:close') { onClose?.(); return; }
    ...
    arcadeOwnsBalance = true;
    if (n === lastPushed) return;
    lastPushed = n;
    acceptingArcadeBalance = true;
    try { setMoney(n); }
    finally { acceptingArcadeBalance = false; }
  };
  iframe.addEventListener('load', () => {
    push({ force: true });
    layer.querySelector('.arcade-chrome')?.remove();
  });
  ...
  return () => {
    ...
    layer.remove();
    mount.release();
    if (activeArcadeLayer === layer) activeArcadeLayer = null;
    if (!mount.portal && !document.querySelector('.arcade-layer')) document.documentElement.classList.remove('has-arcade');
  };
}
