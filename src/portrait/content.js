...
import { hasNativeOverlayPortal } from '../overlay-host.js';
...
export function mountPortraitContent(stage, { onPage } = {}) {
  ...
  const openOverlay = (name, overlayOptions = {}) => {
    ...
    /* Mobile native-flow mounts iframe overlays in a Tavern-body portal, avoiding
       the fixed-floor + cross-origin iframe WebView blackout. Desktop/lifted HUDs
       keep the original page geometry so map/shop/CG/arcade cover the full viewport
       below the browser chrome instead of stopping at the reading pane. */
    const geometry = hasNativeOverlayPortal() ? 'portal' : 'page';
    reportPortraitPage(true, { geometry, page: name, arg: null });
    closeOverlay();
  ...
  const openPage = (page, arg) => {
    ...
    const reportPageOpen = () => reportPortraitPage(true, {
      page, arg: typeof arg === 'string' ? arg : null,
    });
    /* The character archive is taller than one viewport and its stage height is
       measured in requestAnimationFrame. Entering fixed full-screen before that
       measurement leaves iOS with a non-scrollable root for the first touch gesture.
       Other pages retain their established timing; only the archive waits for paint. */
    const delayedCharacterPage = page === 'character';
    const characterSerial = delayedCharacterPage ? ++characterPageSerial : 0;
    if (!delayedCharacterPage) reportPageOpen();
    if (page === 'inventory') {
    ...
    sync();
    if (delayedCharacterPage) {
      /* sync() queued the stage measurement before this callback, so .pstage already
         carries the full scaled archive height when the shell creates the scroller. */
      requestAnimationFrame(() => {
        if (characterSerial !== characterPageSerial || workspace !== page) return;
        reportPageOpen();
        scrollTopTo(0);
      });
    } else {
      /* A page is a new view, not a continuation of the one underneath. */
      scrollTopTo(0);
    }
  };
