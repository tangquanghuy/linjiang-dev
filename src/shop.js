...
export function mountShopOverlay(host, { onClose } = {}) {
  ...
  root.querySelectorAll('.shop-layer').forEach((el) => el.remove());
  root.insertAdjacentHTML('beforeend', shopOverlay());
  const layer = root.querySelector(':scope > .shop-layer');
  activeShopLayer = layer;
  ...
  if (!mount.portal) document.documentElement.classList.add('has-shop');
  ...
  return () => {
    ...
    layer.remove();
    mount.release();
    if (activeShopLayer === layer) activeShopLayer = null;
    if (!mount.portal && !document.querySelector('.shop-layer')) document.documentElement.classList.remove('has-shop');
  };
}
